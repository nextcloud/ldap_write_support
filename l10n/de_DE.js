OC.L10N.register(
    ...
    "Allow users to set their password" : "Benutzer können ihr Passwort selbst festlegen",
    "If the server does not support the modify password extended operation use the `unicodePwd` instead of the `userPassword` attribute for setting the password" : "Wenn der Server den erweiterten Vorgang \"Passwort ändern\" nicht unterstützt, verwenden Sie zum Festlegen des Passworts das Attribut `unicodePwd` anstelle des Attributs `userPassword`.",
    "Use the `unicodePwd` attribute for setting the user password" : "Attribut `unicodePwd` verwenden, um das Benutzerkennwort festzulegen",
